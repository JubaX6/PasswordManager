...
def delete_password():
    service = service_entry.get()
    if service in passwords:
        del passwords[service]  # Delete the entire entry for the service
        messagebox.showinfo("Password Deleted", f"Password for {service} deleted successfully!")
    else:
        messagebox.showwarning("Error", "Password not found.")
...
To delete password, enter Account Name and press "Delete Password"'''
signature = "Developed by Juba Laïche"
...
add_button = tk.Button(center_frame, text="Add Password", command=add_password, height=1, width=12)
add_button.grid(row=5, column=4, padx=12, pady=5)

get_button = tk.Button(center_frame, text="Get Password", command=get_password, height=1, width=12)
get_button.grid(row=6, column=4, padx=12, pady=5)

get_button = tk.Button(center_frame, text="Delete Password", command=delete_password, height=1, width=12)
get_button.grid(row=7, column=4, padx=12, pady=5)

signature_label = tk.Label(center_frame, text=signature, bg="#d3d3d3")
signature_label.grid(row=8, column=1, padx=5, pady=5)
